...
public interface PromotionRepository extends JpaRepository<Promotion, String>, JpaSpecificationExecutor<Promotion> {
    ...
    // find promotion by vendor and user and start date greater than or equal to current date and end date less than or equal to current date
    Set<Promotion> findByVendorAndUsersAndStartDateLessThanEqualAndEndDateGreaterThanEqual(Vendor vendor, User user, LocalDateTime startDate, LocalDateTime endDate);
